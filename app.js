...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Use BIO_DATA alias if available
    const BIO_DATA = window.BIO_DATA || window.MATH_DATA;
    ...
    window.summarizeLesson = () => {
        ...
        const lessonKey = window.currentLessonKey;
        if (!lessonKey) {
            window.typeTerminalMessage("Bio-Sense: Access a specific research module to generate a formal summary.");
            return;
        }

        const summaryData = window.TutorEngine && window.TutorEngine.summarizeContent ? window.TutorEngine.summarizeContent(lessonKey) : "Bio-Sense: Summary module not found.";

        if (typeof summaryData === 'string') {
            window.typeTerminalMessage(summaryData);
        } else {
            let html = `<div style="color:var(--accent-emerald); font-weight:bold; margin-bottom:10px;">📝 ELITE SUMMARY: ${summaryData.title}</div>`;
    ...
    window.submitTutorChat = () => {
        ...
        setTimeout(() => {
            const response = window.TutorEngine && window.TutorEngine.handleChatInput ? window.TutorEngine.handleChatInput(query) : "I'm still learning. Try asking about 'DNA' or 'Genome'!";
    ...
    function showRecommendation() {
        ...
        // Initialize Biology Specifics
        document.title = "Sabrina's Biology Portal | TJHSST IBET Prep";

        const lastSubject = localStorage.getItem('lastSubject_bio') || 'cell-bio';
        const subjectId = window.currentSubjectId || 'cell-bio';
    ...
    function showBadgePopup(chapterTitle) {
        ...
        if (title.includes("unit 1") || title.includes("architecture")) {
            badgeName = "Architect of Life";
            icon = "fa-cube";
        } else if (title.includes("unit 2") || title.includes("gate") || title.includes("permeable")) {
            badgeName = "Osmotic Master";
            icon = "fa-door-open";
        } else if (title.includes("unit 3") || title.includes("enzyme")) {
            badgeName = "Biocatalysis Pro";
            icon = "fa-bolt";
        } else if (title.includes("unit 4") || title.includes("dna architecture")) {
            badgeName = "Helical Explorer";
            icon = "fa-dna";
        } else if (title.includes("unit 5") || title.includes("expression")) {
            badgeName = "Gene Expressionist";
            icon = "fa-code-branch";
        } else if (title.includes("unit 6") || title.includes("operon") || title.includes("biotech")) {
            badgeName = "Control System Analyst";
            icon = "fa-toggle-on";
        } else if (title.includes("unit 7") || title.includes("solar")) {
            badgeName = "Photon Harvester";
            icon = "fa-sun";
        } else if (title.includes("unit 8") || title.includes("respiration")) {
            badgeName = "ATP Sovereign";
            icon = "fa-battery-full";
        } else if (title.includes("unit 9") || title.includes("probability")) {
            badgeName = "Mendelian Strategist";
            icon = "fa-dice";
        } else if (title.includes("unit 10") || title.includes("linkage")) {
            badgeName = "Recombination Expert";
            icon = "fa-project-diagram";
        } else if (title.includes("unit 11") || title.includes("chi-square") || title.includes("inference")) {
            badgeName = "Scientific Invalidator";
            icon = "fa-vial";
            style = "border-color: var(--accent-emerald); box-shadow: 0 0 30px rgba(0, 255, 100, 0.3);";
        } else if (title.includes("unit 13") || title.includes("python") || title.includes("bioinformatics")) {
            badgeName = "Genomic Coder";
            icon = "fa-laptop-code";
        } else if (title.includes("unit 15") || title.includes("evolution")) {
            badgeName = "Darwinian Master";
            icon = "fa-tree";
        ...
        if (container) {
            if (filteredSubjects.length > 0) {
                ...
                if (correctionTerm) {
                    ...
                    notice.innerHTML = `<i class="fas fa-magic"></i> Biometric Correction: Auto-correcting "<strong>${term}</strong>" to "<strong>${correctionTerm}</strong>"`;
    ...
    async function showLesson(lessonKey, subjectId) {
        ...
        try {
            ...
            const nextButtonHtml = nextLesson
                ? `<button class="glass next-btn" onclick="window.finishLesson('${lessonKey}', '${subjectId}'); window.showLesson('${nextLesson.key}', '${subjectId}')" style="padding: 10px 20px; font-weight: 600; color: var(--accent-emerald); display: flex; align-items: center; gap: 8px; border-radius: 12px; border: 1px solid rgba(16, 185, 129, 0.3);">
                ...
                <div class="lesson-view fadeIn">
                    ...
                    <div class="lesson-body-wrapper" style="padding: 0 25px 40px 25px;">
                        ...
                        <div class="lesson-footer" style="margin-top: 50px; text-align: center; padding-bottom: 60px;">
                            ${nextButtonHtml ?
                    `<button class="next-btn-large animate__animated animate__pulse animate__infinite" onclick="window.finishLesson('${lessonKey}', '${subjectId}'); window.showLesson('${nextLesson.key}', '${subjectId}')" style="padding: 18px 45px; font-size: 1.2rem; font-weight: 800; color: #000 !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; border: none; border-radius: 50px; cursor: pointer; box-shadow: 0 10px 30px rgba(16,185,129,0.3); transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: inline-flex; align-items: center; gap: 15px;">
                                    <span>CONTINUE RESEARCH</span>
                                    <i class="fas fa-arrow-right"></i>
                                </button>
                                <p style="margin-top: 15px; color: var(--text-secondary); font-size: 0.9rem;">Next module: <span style="color: var(--accent-emerald); font-weight: 600;">${nextLesson.title}</span></p>`
                    : '<div style="color:var(--text-secondary); font-style:italic; font-size: 1.1rem; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 30px;">Sector Analysis Complete. You have mastered this unit.</div>'
                }
                        </div>
                    </div>
                </div>
    ...
    window.handleJustificationSubmit = (subjectId, unitIdx) => {
        ...
        feedback.innerHTML = `
            <div class="celebration-badge animate__animated animate__tada">
                ...
                <p style="color:white; font-size:0.9rem;">Your reasoning has been verified by the Elite 5.5 Neural Link.</p>
            </div>
        `;
    ...
    window.startDrill = () => {
        ...
        drillTimer = setInterval(() => {
            timeLeft--;
            const t = document.getElementById('drill-timer');
    ...
    const endDrill = () => {
        ...
        if (window.typeTerminalMessage) window.typeTerminalMessage(`DRILL TERMINATED. Final Score: ${drillScore}. ${drillScore > 100 ? "Excellent algorithmic speed detected." : "Precision is high, but throughput needs optimization. Continue recalibrating."}`);
    };
    ...
    function showLibrary() {
        ...
        const libraryData = window.BIO_LIBRARY;
        if (!libraryData) {
            container.innerHTML = '<div class="hero"><h1>Bio-Archive Offline</h1><p>Neural data link interrupted.</p></div>';
            return;
        }

        container.innerHTML = `
            <div class="hero">
                <h1>Bio-Archive <span class="gradient-text">Library</span></h1>
                <p>Curated external intelligence modules and research nodes for TJHSST IBET.</p>
            </div>
                ...
                ${libraryData.categories.map(cat => `
                    <div class="library-category">
                        <h2 style="margin-bottom:25px; display:flex; align-items:center; gap:15px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:15px;">
                            <i class="${cat.icon}" style="color:var(--accent-emerald);"></i> ${cat.title}
                        </h2>
